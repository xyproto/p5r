...
func TestStartingCharsOptionalNegate(t *testing.T) {
	...
	re := MustCompile2(`(^(\S{2} )?\S{2}(\d+|/) *\S{3}\S{3} ?\d{2,4}[A-Z] ?\d{2}[A-Z]{3}|(\S{2} )?\d{2,4})`, 0)
...
func TestFirstcharsIgnoreCase(t *testing.T) {
	...
	re := MustCompile2(`((?i)AB(?-i)C|D)E`, 0)
